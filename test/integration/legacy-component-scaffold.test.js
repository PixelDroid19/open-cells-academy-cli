...
test('CLI 4 Lit 1 and Lit 3 select separate executable trees and lit-component commands', () => {
  ...
  for (const [files, sourceName] of [[lit1, 'legacy-card'], [lit3, 'modern-card']]) {
    ...
    assert.match(helper, /data-event-activity/u);
    assert.match(helper, /data-event-badge/u);
    assert.match(helper, /data-event-toggle/u);
    assert.match(helper, /data-event-popover/u);
    assert.match(helper, /data-event-filter/u);
    assert.match(helper, /data-event-export/u);
    assert.match(helper, /data-event-history/u);
    assert.match(helper, /data-event-clear/u);
    assert.match(helper, /data-event-copy/u);
    assert.match(helper, /width: 768px; height: 480px/u);
    assert.match(helper, /events-panel \{ position: absolute/u);
    assert.doesNotMatch(helper, /events-sidebar|device-status|#f8f3df/u);
    assert.doesNotMatch(files.get('demo/basic.html'), /#f8f3df/u);
    assert.match(files.get('demo/basic.html'), /background: transparent/u);
    assert.match(files.get('demo/demo.js'), /patternLabel: t\(language, 'pattern\.label'\)/u);
