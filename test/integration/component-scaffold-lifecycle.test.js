...
test('component recipe publishes the complete modern component package tree', () => {
  ...
  assert.match(demoHelper, /data-event-activity/);
  assert.match(demoHelper, /data-event-badge/);
  assert.match(demoHelper, /data-event-toggle/);
  assert.match(demoHelper, /data-event-popover/);
  assert.match(demoHelper, /data-event-filter/);
  assert.match(demoHelper, /data-event-export/);
  assert.match(demoHelper, /data-event-history/);
  assert.match(demoHelper, /data-event-clear/);
  assert.match(demoHelper, /data-event-copy/);
  assert.match(demoHelper, /data-label="eventLatest"/);
  assert.match(demoHelper, /data-label="eventHistory"/);
  assert.match(demoHelper, /data-label="clearEvents"/);
  assert.match(demoHelper, /data-label="brandBadge"/);
  assert.match(demoHelper, /data-theme-toggle/);
  assert.match(demoHelper, /data-pattern="dots"/);
  assert.match(demoHelper, /width: 768px; height: 480px/);
  assert.match(demoHelper, /events-panel \{ position: absolute/);
  assert.doesNotMatch(demoHelper, /events-sidebar|device-status|#f8f3df/);
  assert.doesNotMatch(files.get('demo/basic.html'), /#f8f3df/);
  assert.match(files.get('demo/basic.html'), /background: transparent/);
  assert.match(files.get('src/academy-card.css.js'), /border-radius: 1\.25rem/);
  assert.match(files.get('src/academy-card.css.js'), /academy-button-default \{ display: block; width: 100%; \}/);
